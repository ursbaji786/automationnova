...
public class LearnListSet {
	public static void main(String[] args) throws InterruptedException {
		...
		boolean addAlltoSet = trainNum1.addAll(trainNumbers);
		System.out.println("added to set "+addAlltoSet);	
	...
	for (WebElement sortedTrainValues : trainNosAfterClick) {
		...
		int parseIntAfterClick = Integer.parseInt(sorted);
		sortTrainIntArray.add(parseIntAfterClick);
		
		
	}
	
	boolean bothSorted = trainIntArray.equals(sortTrainIntArray);
	System.out.println("both train numbers are same "+bothSorted);	
	
	
	// print train names in alphabetical order (get list of webelements using xpath and then move to Treeset)
